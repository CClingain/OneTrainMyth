...
```{r}
beta_sims <- function(days,noise, seed){
  # Initiate data frame
  data <- NULL
  # Create timestamps for given # of days
  data$timestamp <- rep(times,days)
  ...
  # Create the noise term (input in seconds, converted to minutes)
  set.seed(seed)
  data$noise <- rbeta(length(data$timestamp),shape2 = noise/60,shape1 = 0.5)
   # Convert back to seconds
  data$noise <- data$noise * 60
   # Round for time 
  data$noise <- round(data$noise,0)
...
```{r}
sim1 <- beta_sims(days = 100, noise = 65, seed = 10314)
